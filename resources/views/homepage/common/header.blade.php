          ...
          <div class="flex justify-between items-center">
              <div>
                  <div class="border-r border-primary text-center flex space-x-1 text-white">
                      <span>Tỉ giá tệ</span>
                      <span class="font-bold text-red-600"> {{number_format($cny['content'],0,',','.')}} VNĐ</span>
                  </div>
              </div>
              <div>
                  <div class=" text-center border-r border-primary flex space-x-1 text-white">
                      ...
                      <a class="font-bold text-red-600" href="tel:{{$fcSystem['contact_hotline']}}">{{$fcSystem['contact_hotline']}}</a>
                  </div>
              </div>
          ...
          <div class="container mx-auto px-4">
              <div class="grid grid-cols-12 gap-[30px] items-center">
                  ...
                  <div class="col-span-5">
                      <form class="relative" method="GET" action="{{route('bill.search')}}">
                      ...
                      <div class="px-3 text-center flex space-x-5 items-center">
                          <div class="relative">
                              ...
                              <a href="javascript:void(0)" id="dropdownAvatarNameButton" data-dropdown-toggle="dropdownAvatarName" class="space-x-1 text-base flex items-center font-medium text-gray-900 rounded-full" type="button">
                                  ...
                                  <span>{{Auth::guard('customer')->user()->code}} - <span class="text-red-600">{{number_format(Auth::guard('customer')->user()->price,0,',','.')}} VNĐ</span></span>
                              ...
                              <div id="dropdownAvatarName" class="z-[99999] hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-48 dark:bg-gray-700 dark:divide-gray-600 absolute top-[35px] right-0" style="box-shadow: 0px 8px 15px 0px rgba(0,0,0,0.14);">
                                  <div class="px-4 py-3 text-sm text-gray-900 dark:text-white">
                                      <div class="font-bold text-red-600 text-base">{{Auth::guard('customer')->user()->name}}</div>
                                      <div class="truncate">Số dư: {{number_format(Auth::guard('customer')->user()->price,0,',','.')}} VNĐ</div>
                                  </div>
                                  <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownInformdropdownAvatarNameButtonationButton">
                                      @if($menu_auth && count($menu_auth->menu_items) > 0)
                                      @foreach($menu_auth->menu_items as $key=>$item)
                                      <li>
                                          <a href="{{url($item->slug)}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{$item->title}}</a>
                                      </li>
                                      @endforeach
                                      @endif
                                  </ul>
                                  <div class="py-2">
                                      <a href="{{route('customer.logout')}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Đăng xuất</a>
                                  </div>
                              </div>
                              @else
                              <a href="{{route('customer.login')}}" class="space-x-1 flex items-center text-sm font-medium text-gray-900 rounded-full hover:text-blue-600 " type="button">
                          ...
                          <a class="relative tp-cart" href="{{url('carts')}}">
                              ...
                              <span class="cart-quantity absolute rounded-full right-[-9px] top-[-9px] text-white text-xs w-5 h-5 bg-black flex items-center justify-center" aria-hidden="true">{{$cart}}</span>
                          </a>
                      </div>
                  </div>
              </div>
              ...
              <div class="flex justify-between items-center">
                  <ul class="flex menu_header">
                      ...
                      <li class="group relative">
                          <a href="{{url($item->slug)}}" class="px-[15px] text-white font-medium flex items-center space-x-1 text-base">
                          ...
                          <div class="absolute top-full left-0 bg-white p-[10px] w-[300px] shadow menu-header-child z-[999999] hidden group-hover:block">
                              <ul class="">
                                  ...
                                  <li class="border-b border-dashed border-primary w-full float-left">
                                      <a href="{{url($child->slug)}}" class="py-2 w-full hover:text-primary float-left">{{$child->title}}</a>
                                  </li>
                                  @endforeach
                              </ul>
                          </div>
  ...
  <header class="block lg:hidden relative">
      <div class="flex justify-center px-2 py-[5px] header-22">
          <div class="w-full text-center">
              ...
              <a href="{{getUrlHome()}}" class="logo">
                  <img src="{{asset($fcSystem['homepage_logo'])}}" class="inline-block max-w-[200px]" alt="{{$fcSystem['homepage_company']}}" />
              </a>
              <div class="absolute right-[15px] top-1/2 -translate-y-1/2 z-[99]">
                  <div class="relative">
                      ...
                      <div id="dropdownAvatarName" class="z-[99999] hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-48 dark:bg-gray-700 dark:divide-gray-600 absolute top-[35px] right-0" style="box-shadow: 0px 8px 15px 0px rgba(0,0,0,0.14);">
                          <div class="px-4 py-3 text-sm text-gray-900 dark:text-white">
                              <div class="font-bold text-red-600 text-base">{{Auth::guard('customer')->user()->code}}</div>
                              <div class="truncate">Số dư: {{number_format(Auth::guard('customer')->user()->price,0,',','.')}} VNĐ</div>
                          </div>
                          <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownInformdropdownAvatarNameButtonationButton">
                              @if($menu_auth && count($menu_auth->menu_items) > 0)
                      ...
                      @else
                      <a href="{{route('customer.login')}}" class="flex items-center text-sm font-medium text-gray-900 rounded-full hover:text-blue-600 " type="button">
                          <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                              <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0 0a8.949 8.949 0 0 0 4.951-1.488A3.987 3.987 0 0 0 11 14H9a3.987 3.987 0 0 0-3.951 3.512A8.948 8.948 0 0 0 10 19Zm3-11a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                          </svg>
                      </a>
