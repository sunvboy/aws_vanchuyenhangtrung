...
class PackagingController extends Controller
{
    ...
    {
        $sessions = Session::get('PackagingController');
        $id = $request->id;
        if (!empty($sessions)) {
            $data = Warehouse::where('deleted_at', null)->where('publish', 0)->select('id', 'code_cn', 'weight', 'code_vn', 'customer_id', 'publish')
                ->whereIn('code_vn', $sessions)->orderBy('id', 'asc')->get();
        }
        $products = [];
        if (!empty($data)) {
            foreach ($data as $item) {
                $products['code'][] = $item->code_cn;
                $products['code_vn'][] = $item->code_vn;
                $products['id'][] = $item->id;
                $products['weight'][] = $item->weight;
            }
        }
        $module = $this->table;
        ...
        if (!empty($lastRow)) {
            ...
            $code = $str . '1' . str_pad($lastId, 3, '0', STR_PAD_LEFT);
        } else {
            $code = $str . '1001';
        } */
        $detail = [];
        if (!empty($id)) {
            $detail = Packaging::with('packaging_relationships')->find($id);
        }
    ...
    public function printer(Request $request)
    {
        ...
        return response()->json(['detail' => $detail, 'code' => $code, 'count' => $count]);
    }
