...
class PackagingsFrontendExport implements FromCollection, WithHeadings, WithMapping, WithEvents, WithColumnWidths
{

    public function collection()
    {

        $data = \App\Models\Packaging::where('deleted_at', null)->where('customer_id', Auth::guard('customer')->user()->id)->with(
            ['packaging_relationships' => function ($query) {
                $query->with('warehouses_vietnam');
            }]
        )->with('packaging_v_n_s')->orderBy('id', 'desc')->groupBy('code');
        $tmp = [];
        if (!empty($_GET['ids'])) {
