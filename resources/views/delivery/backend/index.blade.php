...
<div class="content ">
    <div class="flex justify-between items-center mt-5">
        ...
        <div class="flex">
            @can('deliveries_create')
            <a href="{{route('deliveries.create')}}" class="btn btn-primary shadow-md mr-2">{{trans('admin.create')}}</a>
            @endcan
    ...
    <div class="grid grid-cols-12 gap-6 mt-2">
        <div class="col-span-12 mt-2">
            <form action="" class="grid grid-cols-12 gap-2" id="search" style="margin-bottom: 0px;">
                @can('deliveries_destroy')
                <div class="col-span-2">
                    <select class="form-control ajax-delete-all-deliveries h-10" data-title="Lưu ý: Khi bạn xóa danh mục nội dung tĩnh, toàn bộ nội dung tĩnh trong nhóm này sẽ bị xóa. Hãy chắc chắn rằng bạn muốn thực hiện chức năng này!" data-module="{{$module}}">
        ...
        <div class="col-span-12 overflow-auto space-y-2">
            <div class="font-bold" style="border-radius: 0px;color: #000"><span class="text-danger">Tổng dòng</span> : {{!empty($count)?$count : 0}} </div>
            <div class="font-bold" style="border-radius: 0px;color: #000"><span class="text-danger">Tổng cân 总重量</span>: {{number_format((float)$total_weight, 2, '.', '')}}</div>
            <div class="font-bold" style="border-radius: 0px;color: #000"><span class="text-danger">Tổng tiền</span>: {{number_format((float)$prices, 0, ',', '.')}} VNĐ</div>
            ...
            <table class="table table-report">
                <thead class="table-dark">
                    ...
                    <tr>
                        ...
                        <th>{{trans('admin.code_deliveries')}}</th>
                ...
                <tbody>
                    @foreach($data as $v)
                    <tr class="odd odd-{{$v->id}} odd-export-warehouse-{{$v->id}}" data-status="{{$v->status}}" id="post-<?php echo $v->id; ?>">
                        <td>
                        ...
                        <td>
                            {{$v->created_at}}<br><span class="font-bold">({{!empty($v->user) ? $v->user->name : ''}})</span>
                        </td>
                        <td>
                            <a href="javascript:void(0)" style="text-decoration: underline;" data-status="{{$v->status}}" data-image="" data-code="{{$v->code}}" data-id="{{$v->id}}" data-price="{{floor($v->price)}}" class="ja_modalPayment  ja_modalPayment_<?php echo $v->id ?> font-bold text-success">{{$v->code}}</a>
                        </td>
                        ...
                        <td>
                            <div class="flex">
                                <input type="text" style="width: 100px" class="form-control flex-1 form-weight form-weight-{{$v->id}}" data-id="{{$v->id}}" value="{{$v->weight}}" @if($v->status != 'wait') disabled @endif>
                            </div>
                        </td>
                        <td>
                            <div class="flex">
                                <input type="text" style="width: 100px" class="form-control flex-1 form-fee form-fee-{{$v->id}} int" data-id="{{$v->id}}" value="{{number_format($v->fee,'0',',','.')}}" @if($v->status != 'wait') disabled @endif>
                            </div>
                        </td>
                        <td>
                            <div class="flex">
                                <input type="text" style="width: 100px" class="form-control flex-1 form-shipping form-shipping-{{$v->id}} int" data-id="{{$v->id}}" value="{{number_format($v->shipping,'0',',','.')}}" @if($v->status != 'wait') disabled @endif>
                                @if($v->status == 'wait')
                                <button type="button" class="w-10 js_updateTotal btn btn-success" data-id="{{$v->id}}" style="height: 42px;padding: 0">
                        ...
                        <td>
                            <span class="form-total-{{$v->id}} text-danger font-bold">
                        ...
                        <td>
                            ...
                            <button class="btn btn-primary btn-sm mt-1">#{{$v->code_merge}}</button>
                            @endif
                        </td>
                        ...
                        <td >
                            ...
                            <div style="display: none;">
                                @if(!empty($v->advanced))
                                    <textarea class="textareacopy-{{$v->id}}">@if(!empty($products) && count($products) > 0) @foreach($products as $item){{$item}}&#13;&#10;@endforeach @endif</textarea>

                                @else
                                    <textarea class="textareacopy-{{$v->id}}">@if(!empty($products) && count($products) > 0)@if(!empty($products['code']) && count($products['code']) > 0)@foreach($products['code'] as $item){{$item}}&#13;&#10;@endforeach @endif @endif</textarea>

                                @endif
                            </div>
                            <div class="space-y-5">
                                <div class="flex">
                                    <a class="flex items-center mr-3 text-base" href="javascript:void(0)" onclick="getinfo({{$v->id}})">
                                        <i data-lucide="printer" class="w-4 h-4 mr-1"></i> Print
                                    </a>
                                    <a class="flex items-center mr-3 text-base" href="javascript:void(0)" onclick="copyToClipboard('.textareacopy-{{$v->id}}')">
                                ...
                                <div class="flex">
                                    @can('deliveries_edit')
                                        @if(!empty($v->advanced))
                                            <a class="flex items-center mr-3 text-base" href="{{route('deliveries.advanced',['id'=>$v->id]) }}">
                                                <i data-lucide="check-square" class="w-4 h-4 mr-1"></i>
                                                Edit
                                            </a>
                                        @else

                                            <a class="flex items-center mr-3 text-base" href="{{route('deliveries.edit',['id'=>$v->id]) }}">
                                                <i data-lucide="check-square" class="w-4 h-4 mr-1"></i>
                                                Edit
                                            </a>
                                        @endif
