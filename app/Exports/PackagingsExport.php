...
class PackagingsExport implements FromCollection, WithHeadings, WithMapping, WithEvents, WithColumnWidths
{

    public function collection()
    {

        $data = \App\Models\Packaging::where('deleted_at', null)->with(['packaging_relationships' => function ($query) {
            $query->with('warehouses_vietnam');
        }])->with(['customer'])->orderBy('id', 'desc')->groupBy('code');
        $tmp = [];
        if (!empty($_GET['ids'])) {
        ...
        if (!empty($data) && count($data) > 0) {
            foreach ($data as $key => $item) {
                ...
                if(!empty($itempackaging_v_n_s)){
                    $status = 'Đã về VN';
                }elseif(!empty($item->packaging_trucks)){
                    $status = 'Xếp xe TQ';
                }else{
                    $status = 'Nhập kho TQ';
                }
