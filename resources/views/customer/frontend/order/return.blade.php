            ...
            <div class="flex-1 overflow-x-hidden shadowC rounded-xl w-full md:w-auto order-1 md:order-2">
                <div class="p-6 bg-white space-y-3 pb-[150px]">
                    <div class="flex md:items-center flex-col md:flex-row justify-between">
                        <h1 class="text-black font-bold text-xl">Hoàn tiền #{{$detail->code}}</h1>
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="space-y-3 p-5 border rounded-md shadow">
                            <div class="space-y-3" id="formData">
                                ...
                                @if(!empty($link))
                                <div class="space-y-3 ">
                                    <label for="checkbox-{{$key}}" class="flex border-b pb-2 cursor-pointer items-center relative overflow-hidden">
                                        <input type="checkbox" name="links[]" id="checkbox-{{$key}}" class="w-4 h-4 checkbox-item-quyen checkboxC" value="{{$link}}">
                        ...
                        <div class="grid grid-cols-1 space-y-3">
                            ...
                            <div class="grid grid-cols-1 flex-wrap gap-3">
                                ...
                                @if(!empty($img))
                                <label for="checkbox2-{{$key}}" class="flex border-b pb-2 cursor-pointer items-center relative">
                                    <input type="checkbox" name="images[]" id="checkbox2-{{$key}}" class="w-4 h-4 checkbox-item-quyen-2 checkboxC" value="{{$img}}">
                                    <div>
                                        <img src="{{asset($img)}}" class="h-[70px] object-cover pl-2 flex-1" />
                                    </div>
                                </label>
                                @endif
                                @endforeach
                        ...
                        <div class="grid grid-cols-5 pb-2 space-x-3 items-center">
                            ...
                            <input class="col-span-3 outline-none focus:outline-none hover:outline-none border rounded-md h-10 px-4 int" name="price_return" value="{{number_format($detail->total_price_vnd_final,'0',',','.')}}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col lg:flex-row items-start lg:space-x-8">
                ...
                <div class="flex-1 flex justify-between items-center overflow-x-hidden w-full md:w-full order-1 md:order-2 py-5 text-white bg_gradient p-4 md:rounded-lg">
                    <div class="w-1/2 flex flex-col space-y-1">
                        <div>Số tiền có thể hoàn(VNĐ): <span class="font-bold total_price_vnd">{{number_format($detail->total_price_vnd_final,'0',',','.')}}</span></div>
                    </div>
                    <div class="w-1/2 space-x-1 justify-end flex flex-wrap">
                        <a href="javascript:void(0)" style="background-color: {{config('cart')['class']['returns']}};" class="js_handleSubmit text-white outline-none focus:outline-none hover:outline-none font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5">Xác nhận</a>
                    </div>
                </div>
            </div>
        </div>
