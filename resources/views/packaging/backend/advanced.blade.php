    ...
    <div class="content hidePrint">
        <div class=" flex items-center justify-between mt-8">
            ...
            <div class="flex space-x-1">
                <a href="{{route('packagings.create', ['id' => $detail->id, 'customer_id' => $detail->customer_id])}}" class="btn btn-primary text-white ">
                     Tạo bao  集包
                </a>
        ...
        <form class="grid grid-cols-12 gap-6 mt-5" role="form" action="{{route('packagings.store')}}" method="post" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <select name="customer_id" class="form-control tom-select tom-select-custom w-full">
                                ...
                                @foreach($customers as $k=>$v)
                                    <option data-name="{{$v->name}}" value="{{$v->id}}" {{ !empty($detail->customer_id == $v->id) ? 'selected':'';}}>
                                        {{$v->code}} - {{$v->name}}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class=" col-span-12 lg:col-span-12">
                <table class="table table-report -mt-2">
                    ...
                    <tr class="odd">
                        ...
                        <td>{{$detailOld->created_at}}</td>
                        <td>
                            <a href="{{route('packagings.show',['id' => $detailOld->id])}}" class="text-danger font-bold">{{$detailOld->code}}</a>
                        </td>
                        <td>
                            <a href="{{route('packagings.show',['id' => $detailOld->id])}}" target="_blank" class="text-primary font-bold" style="text-decoration: underline;">Mã vận đơn</a>
                        </td>
                        <td>
                            {{$detailOld->total_weight}}
                        </td>

                        <td>
                            {{!empty($detailOld->customer) ?$detailOld->customer->code:''}}
                        </td>
                        <td>
                            {{!empty($detailOld->customer) ?$detailOld->customer->name:''}}
                        </td>
                        <td class="">
                ...
                <table style="width:320px;height:200px;border:1px solid #000;font-size:20px !important;text-align:center">
                    ...
                    <tr>
                        <td style="border:1px solid #000;font-size:18px">
                            ...
                            <p style="font-size:18px;text-align:center">{{!empty($detailOld->customer) ?$detailOld->customer->code:''}} - {{!empty($detailOld->customer) ?$detailOld->customer->name:''}} / {{number_format((float)$detailOld->value_weight, 2, '.', '')}} kg</p>
                            <p style="font-size:18px;text-align:center"> Số kiện: <span>{{$detailOld->packaging_relationships->count()}}</span></p>
                        </td>
                    </tr>
                    <tr>
                        <td></td>
