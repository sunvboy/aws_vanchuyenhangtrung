...
class WarehouseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //         'CCCD' => vnfaker()->int(),
        //         'zip' => vnfaker()->float($min = 10000, $max = 11000, $decimals = 0)
        //     ];
        // }
        // CustomerFake::insert($tmp);
        // dD($tmp);

        $module = $this->table;
        $data = Warehouse::orderBy('created_at', 'desc')->with('user')
            ->with('customer')
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            // 'name_vn.required' => 'Tên sản phẩm là trường bắt buộc.',
        ...
        if (!empty($lastRow)) {
            ...
            $code =  $str . Auth::user()->id . date('Ymd')  . str_pad($lastId, 4, '0', STR_PAD_LEFT);
        } else {
            $code = $str . Auth::user()->id . date('Ymd') . '0001';
        }
        $quantity_kien = $request->quantity_kien;
        if ($quantity_kien > 1) {
            $_insert = [];
            for ($i = 1; $i <= $quantity_kien; $i++) {
                $customer = CustomerFake::inRandomOrder()->first();
                $_insert[] = [
        ...
        /* if ($id > 0) {
            ...
            WarehouseRelationships::insert($_insert);
        }*/
    ...
    public function update(Request $request, $id)
    {
        $request->validate([
            'customer_id' => 'required',
            // 'code_cn' => ['required', Rule::unique('warehouses')->where(function ($query) use ($id) {
            ...
            //            'quantity_kien.required' => 'Số lượng sản phẩm 数量产品 là trường bắt buộc.',
            //            'quantity_kien.numeric' => 'Số lượng sản phẩm 数量产品 phải là số.',
            //            'quantity_kien.min' => 'Số lượng sản phẩm 数量产品 lớn hơn.',
        ...
        /* WarehouseRelationships::where('warehouse_id', $id)->delete();
        $_insert = [];
        for ($i = 1; $i <= $quantity_kien; $i++) {
            $_insert[] = [
                'warehouse_id' => $id,
                'code' =>  $request->code_vn . '-' . $i,
    ...
    public function printer(Request $request)
    {
        ...
        return response()->json(['detail' => $detail, 'code' => $code, 'str' => $str]);
    }
    public function printer_all(Request $request)
    {
        ...
        if (!$data->isEmpty()) {
         ...
         <tr>
             <td style="border:1px solid #000;text-align: center;font-weight: bold;" colspan="3" rowspan="2" class="">
' . $item->code_cn . '
             </td>
             <td style="border:1px solid #000;text-align:center">Ngày</td>
         </tr>
         <tr>
             <td class="" style="border:1px solid #000;text-align: center;">
' . $item->date . '
             </td>
         </tr>
         <tr>
