                                        ...
                                        <table class="min-w-full text-sm font-light">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="border-b dark:border-neutral-500">
                                                    <td class="whitespace-nowrap  p-2"> {{$data->firstItem()+$loop->index}}</td>
                                                    <td class="whitespace-nowrap  p-2 ">
                                                        <p>{{$v->created_at}}</p>
                                                        <p class="font-bold">{{$v->note}}</p>
                                                    </td>
                                                    <td class="whitespace-nowrap  p-2 font-bold">{{number_format($v->price_old,'0',',','.')}}</td>
                                                    @if($v->price_final-$v->price_old < 0) <td class="whitespace-nowrap  p-4 font-bold text-red-600">{{number_format($v->price_final-$v->price_old,'0',',','.')}}</td>
                                                        @else
                                                        <td class="whitespace-nowrap  p-2 font-bold text-green-500">+ {{number_format($v->price_final-$v->price_old,'0',',','.')}}</td>
                                                        @endif

                                                        <td class="whitespace-nowrap  p-2 font-bold">{{number_format($v->price_final,'0',',','.')}}</td>
                                                </tr>
                                                @endforeach
                                                @endif
                                            </tbody>
