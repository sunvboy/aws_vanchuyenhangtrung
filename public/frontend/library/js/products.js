...
$(function () {
  ...
  $(document).on("click", ".swatch-option", function () {
    ...
    _this.addClass("done");
    let count_version = __this.find(".tp_addToCart").attr("data-count-version");
        ...
        function (data) {
          ...
          if (data.getVersionproduct.status_version == 1) {
            ...
            __this
              .find(".card-price")
              .html(
                numberWithCommas(data.getVersionproduct.price_version) + "₫"
              );
            //thực hiện add attr giỏ hàng
            __this
              .find(".tp_addToCart")
              .attr("data-price", data.getVersionproduct.price_version);
            __this
              .find(".tp_addToCart")
              .attr("data-title-version", data.getVersionproduct.title_version);
            __this
              .find(".tp_addToCart")
              .attr("data-id-version", data.getVersionproduct.id_sort);
          }
        }
      );
      return false;
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (json.error == "") {
          ...
          loadDataCart(json);
          $('li[data-rowid="' + rowid + '"] .total-product-item').html(
            numberWithCommas(json.total_item) + "₫"
          );
